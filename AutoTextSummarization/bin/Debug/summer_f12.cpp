...
/* FUNCTION PROTOTYPES */
void basic_in_map(char[]);//prototype of a fun which inserts words into basic map
...
void lineextract(char[],char[]); //extracts a line from a node and stores it in a node
void extraspace(char[]);//adds an extra space at the end of the file...this is necessary to extract the last line
void extractword(line*,char[]);//extracts a word from a sentence.
...
void sort_fun(sorter[],int);//sorts the line_sort_array[]
...
{
     //cout<<summary_factor<<" "<<countLine;
     ofstream output;
     output.open(destinationfile, ios::out);
     output<<" ";//jugad for printing no summary :P
     line* ptr=new line;
     ptr=start;
...
void sort_fun(sorter line_sort_array[],int summary_factor)
{
     ...
     //cout<<"#";
     int summary_length=(countLine*summary_factor*10)/100;
      for(int i=0;i<summary_length;i++)
     {
             for(int j=0;j<i;j++)
             {
...
void basic_in_map(char article_type[])
{
     //cout<<article_type;
    char checkarray[15];
    string str;
    ifstream input;
    if(strcmp(article_type,"general")==0)
        input.open(".\\common_words.txt");
    else if(strcmp(article_type,"sports")==0)
         input.open(".\\sports.txt");
     else if(strcmp(article_type,"business")==0)
         input.open(".\\business.txt");
      else if(strcmp(article_type,"science")==0)
         input.open(".\\science.txt");
       else if(strcmp(article_type,"political")==0)
         input.open(".\\political.txt");
         //cout<<".";
    while(1)
    {
                       checkarray[0]='\0';
...
void extractword(line* ptr,char article_type[])
{
     ofstream input;
    if(strcmp(article_type,"general")==0)
        input.open(".\\common_words.txt",ios::app);
    else if(strcmp(article_type,"sports")==0)
         input.open(".\\sports.txt",ios::app);
     else if(strcmp(article_type,"business")==0)
         input.open(".\\business.txt",ios::app);
      else if(strcmp(article_type,"science")==0)
         input.open(".\\science.txt",ios::app);
       else if(strcmp(article_type,"political")==0)
         input.open(".\\political.txt",ios::app);
       ...
       for(int i=0;i<len;i++,j++)
       {
               if(sentence[i]==' '||sentence[i]==char(10))
               {
                                ...
                                if(noun==1)
                                          ...
                                          float keyscore=i1->second;//keyscore stores the count of word in first sentence
                                          ptr->score=ptr->score+2+(keyscore/4)+(freq/100);//add the noun score to the sentence
                                          }
                                else
                                {
                                ...
                                int flag=0;//for controling goto
                                map<string,int>::iterator i;
                                stemmer_check://goto label
                                //cout<<word<<strlen(word)<<endl;
                                i=basicWord.find(word);
                                if(i!=basicWord.end())
                                         ...
                                         {//if the word is a number
                                              int numeral = checkNumeral(word);  
							  		 		    ...
							  		 		    {   // +2 if numeral
											 		   ...
											 		   map<string,int>::iterator i=wordFrequency.find(word);
											 		   float freq = i->second;   
											 		   ptr->score=ptr->score+2+(keyscore/4)+(freq/100);   	   		   
								 		   		}
						 		   		     else
						 		   		     {
                                             //filter the word using stemmer and again search the maps
                                            if(flag==0){
                                                         flag++;
                                                         strcpy(word,stemmer(word));
                                                         //cout<<word;
                                                         goto stemmer_check;
                                                         }
                                             ...
                                             float freq=i->second;
                                             //------------------------------------------------------
                                             //adding common words using AI
                                             //input<<"\n"<<word;
                                             //------------------------------------------------------
                                             ptr->score=ptr->score+1.5+(keyscore/4)+(freq/100);
                                             }	 	 
						 			}	
                                }
                                 }
...
void lineextract(char sourcefile[],char article_type[])
{
     char heading[1000];
     heading[0]='\0';
    ...
    for(ptr=start;ptr->next!=NULL;)
     {                            
     extractword(ptr,article_type);//sentence is passed to extract word
...
//int main()
{
    ...
    sort_fun(line_sort_array,atoi(argv[3]));
